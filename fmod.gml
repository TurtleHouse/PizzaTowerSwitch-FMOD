/*
this file gives gml contents to every fmod function the pizza tower switch port (v1.0.3 SR 1) uses. by the popular demand of two (2) people, here's a public version with some comments
...
function fmod_update() {
	...
	global.totemmult = Approach(global.totemmult, global.totemfade / 5, 0.1)
...
function d_get_soundlist(soundname) { 
	switch (soundname) {
		...
		case "sfx/pep/mach":
			return [sfx_pep_rollgetup, sfx_mach1, sfx_mach2, sfx_mach3, sfx_mach4]
		case "sfx/voice/myea":
		...
		case "sfx/pizzahead/finale":
			return [sfx_pizzahead_finale, windloop]
		...
		case "music/pizzatime":
			return [mu_pizzatime, sfx_enemies_johndead, mu_chase]
		case "music/hub":
	...
	switch (soundname) {
		...
		case "sfx/pep/mach":
			...
			function() {
				...
				if t_state >= 0 {
					var ind = t_state
					var playingsound = audio_play_sound_on(emitter, soundlist[ind], looping, 10)	
					ds_list_add(sound_instances, playingsound)
		...
		case "sfx/misc/hamkuff":
			...
			function() {
				...
				if newstate == 2
					...
					doloop = true
				var playingsound = play_sound(soundlist[newstate], doloop, 1)
				ds_list_add(sound_instances, playingsound)	
		...
		case "sfx/ratmount/mach":
			...
			function(grounded) {
				...
				if fmod_event_instance_is_playing(self) {
					...
					fmod_event_instance_stop(self)
					var playingsound = audio_play_sound_on(emitter, soundlist[ind], looping, 10, 1, offset)	
					ds_list_add(sound_instances, playingsound)
					change_loop_sound(playingsound)
				}
				t_anyvar1 = grounded
		...
		*/
		...
		case "sfx/pizzahead/finale":
			...
			function(newstate) {
				...
				if pause {
					sndgain = 1.5
					t_anyvar1 = play_sound(soundlist[1], false, 1)
					sndgain = 1
		...
		case "music/title":
			return new fmod_sound(soundlist, undefined, undefined, undefined, true, [0, 4.83], undefined, function(playingsound = noone) {
				statetimer = 0	
				change_loop_points([0, 4.83])
				if audio_is_playing(playingsound) {
					audio_sound_gain(playingsound, 0.6, 0)	
					t_anyvar1 = playingsound
				}
			}, undefined, function() {
				if (t_state == 1) {
					if statetimer == 0 && audio_is_playing(t_anyvar1) 
						audio_sound_gain(t_anyvar1, 1, 0)
					statetimer += 1
					if statetimer == 20	
		...
		case "music/pizzatime":
			...
			function () {
				var playingsound = play_sound(soundlist[0], false, 1)
				ds_list_add(sound_instances, playingsound)	
				playingsound = play_sound(soundlist[1], false, 1)
				ds_list_add(sound_instances, playingsound)	
			}, 
			function(newstate) {
				if (newstate == 1) {
					audio_sound_gain(sound_instances[| 0], 0, 1000)
					var playingsound = audio_play_sound(soundlist[0], 10, false, 0, 170.7)
					audio_sound_gain(playingsound, 1, 1000)
					sound_instances[| 1] = playingsound // overrides the pillar sound but it's long done by now
				}
				else if (newstate == 2) {
					audio_sound_gain(sound_instances[| 0], 0, 500)
					audio_sound_gain(sound_instances[| 1], 0, 500)
				...
				if !is_undefined(sound_instances[| 0]) {
					...
					if pos > 165 + 1/3 
						audio_sound_set_track_position(sound_instances[| 0], (165 + 1/3) - pos + 53 + 1/3)
				}
				if !is_undefined(sound_instances[| 2]) {
					pos = audio_sound_get_track_position(sound_instances[| 2])
					if pos > 168.85
						audio_sound_set_track_position(sound_instances[| 2], (168.85) - pos + 40.85)
				}
		...
		case "music/w5/kidspartychase":
			return new fmod_sound(soundlist, undefined, undefined, undefined, true, [-1, -1], true,
			function() {
				t_anyvar1 = 26
		...
		case "music/boss/noise":
			return new fmod_sound(soundlist, undefined, undefined, undefined, true, [-1, -1], undefined, undefined, undefined, function() {
				...
				if trackpos >= (60 * 2 + 13.71) {
					...
					var endsound = audio_play_sound(Pizza_Tower_OST___Pumpin_Hot_Stuff, 10, false, 1, trackpos)
				}
			})
		case "music/boss/fakepepambient":
