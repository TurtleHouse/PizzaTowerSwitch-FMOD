/*
see comments in fmod.gml.
in the pizza tower switch port, this runs in obj_fmod's end step.
*/
	...
	with (soundwatchers[| i]) {
		...
		if sound != noone {
			if loop_points != [-1, -1] and audio_is_playing(sound) {
				...
				if abs(audio_point - last_time_point) > 1 and audio_point == 0 { // safety check. on switch, the function above seems to sometimes return 0 for a frame. god knows why.
					fail_count++
					if fail_count > 1
...
// just realized i did not include a line here to calculate the length of the audiostoppers list. this is a bug in the current version of the switch port
// and i didn't notice it. it was using the soundwatcher len the whole time. this should definitely work (but i've never tested)
